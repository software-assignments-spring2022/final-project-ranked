    ...
    return (
        <ul>
            {filteredGames.map((item) => (
                ...
                <li key={item._id} onClick={() => handleClick({_id: item._id})} className="search-result-game">{item.gamename}</li>
            ))}
            {filteredPosts.map((item) => (
                ...
                // original code
                // this returns the game-name,posts
                <li key={item._id} onClick={() => handleClick2({_id: item._id, megathread: item.toMegathread})} className="search-result-post">{item.title}</li>
            ))}
        </ul>
    )
}
